...
| `tools/quality/broadridge-visual-check.mjs` | Playwright + pixelmatch visual diff at 300px mobile (`<600px` CSS), 600px tablet (`600–899px`), and 1200px desktop (`≥900px`); writes before/after/diff PNGs. Reads pages from `config.targets`; skips 404 targets. | `broadridge:test:visual` → **dev locally + the visual matrix PR workflow**. |
