...
**Honest critique of the numbers:**
- The prior "8-core-pages" framing **understated total scope** because the fund pages were speculatively excluded instead of verified as internal. That was the biggest gap. Now closed.
...
- **Plugin:** PDF generation uses the **project-management** plugin, now enabled in `.agents/settings.json`. The PDF is generated via the project-management skill once the plugin has loaded.
...
### Immediate actions — enable plugin + create document — ✅ complete
- [x] Write `.agents/settings.json` with `{"enabledPlugins": {"project-management@excat-extended": true}}` to enable the project-management plugin.
- [x] Save this artifact to `docs/cit-migration-plan.md` (editable Markdown source).
- [x] Generate the polished stakeholder **PDF** via the project-management skill → `docs/cit-migration-plan.pdf` (7-page Adobe-branded).
- [x] Confirm both files exist (`docs/cit-migration-plan.md` + `docs/cit-migration-plan.pdf`).
...
### Phase 1 — Foundation & design system (M1) — ✅ complete
- [x] Start local dev server (`aem up`, background); confirm DA project type and preview/prod URLs.
- [x] Create migration feature branch off `main` (`migrate-cit-section`); verify migration profile targets `da-broadridge-cit`.
- [x] Scrape landing page (content, metadata, images); confirm section boundaries. *(artifacts in `migration-work/`; 6 content sections + shared chrome confirmed)*
- [x] Extract design tokens (colors, fonts, spacing) into global styles. *(brand blue `#0050ae`, navy `#001f5a`, accent `#40b8ff`, text `#16161d`; light-weight headings; solid-blue small-radius buttons — applied to `styles/styles.css`)*

### Phase 2 — Shared chrome & blocks (M2) — 🔄 in progress
- [x] Build header/nav (`nav` + `header`) with the simple 4-link MATRIX SOLUTIONS dropdown, responsive + mobile. *(boilerplate nav supports `nav-drop`; brand CSS applied — uppercase links, brand-blue hover, dropdown accent)*
- [x] Build footer (`footer`) with two link groups, social icons, legal links. *(navy `#001f5a` bg, white/accent links, column layout)*
- [x] Build `hero-banner` block + image/CTA variants. *(centered, dark scrim for legibility over photo)*
- [x] Build `cards-feature` block. *(solid brand-blue clickable tiles, white text — matches source)*
- [x] Build `columns-media` + `columns-compare` blocks (characteristics + two-column comparison lists with brand check/cross bullets).
- [x] Build **form-contact** block (Full Name, Country, Comments, "Contact Sales" submit, success message, phone + FAQ). *(client-side)*
- [x] Scope all block CSS; verify responsive at 600/900/1200 breakpoints. *(lint clean)*
- [ ] `offerings-grid` block (22 image cards + links) — deferred to Phase 4 (matrix-cits page).
...
---
*Planning/estimation artifact — fact-checked across three re-scraping passes and self-critiqued (nav simplified, fund pages brought in scope).*
